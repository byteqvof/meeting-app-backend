...
async function uploadAvatar(
  ...
  const path =
    `${profileId}/avatar-${Date.now()}-${crypto.randomUUID()}.${extension}`
  const { error } = await supabaseAdmin.storage.from(AVATAR_BUCKET).upload(
    path,
    file,
    {
      cacheControl: '3600',
...
async function removeAvatarByPath(
  ...
  const { error } = await supabaseAdmin.storage.from(AVATAR_BUCKET).remove([
    path,
  ])
...
export default {
  fetch: withSupabase({ auth: 'user' }, async (req, ctx) => {
    ...
    try {
      if (req.method === 'GET') {
        const profileId = optionalUuid(url.searchParams.get('id'), 'id') ?? userId
        ...
        if (error) {
          logger.error('profile_fetch_failed', { profile_id: profileId, error })
          return errorResponse(
      ...
      if (req.method === 'POST') {
        ...
        const { error: insertError } = await ctx.supabase.from('profiles').insert({
          id: userId,
          ...profilePayload,
        })
        ...
        if (insertError) {
          ...
          return errorResponse(
            status === 409 ? 'Profile already exists' : 'Could not create profile',
            status,
            insertError,
            responseHeaders,
          )
      ...
      if (req.method === 'PATCH' || req.method === 'PUT') {
        ...
        let uploadedAvatarPath: string | null = null
        const oldAvatarPath = avatarPathFromPublicUrl(existingProfile.avatar_url)
      ...
      if (req.method === 'DELETE') {
        ...
        if (error) {
          logger.error('profile_delete_failed', { error })
          return errorResponse(
